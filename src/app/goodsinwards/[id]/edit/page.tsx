...
const EditGoodsInward: React.FC<PageProps> = ({ params }) => {
  ...
  // Fetch existing goods inward data
  useEffect(() => {
    const fetchGoodsInward = async () => {
      try {
        ...
        setType(data.type || '');
        setCustDcNo(data.custDcNo || '');
        setDcDate(data.dcDate ? data.dcDate.split('T')[0] : '');
        setCustOrdNo(data.custOrdNo || '');
        setVehNo(data.vehNo || '');
        setStyleRefNo(data.styleRefNo || '');
        setGreigeMarkingId(data.greigeMarkingId || '');
        setMarketingBy(data.marketingBy || '');
        setPreparedBy(data.preparedBy || '');
        setCustOrdIncharge(data.custOrdIncharge || '');
        setOldDoNo(data.oldDoNo || '');
        setMajorFabric(data.majorFabric || '');
        setLabNo(data.labNo || '');
        setNarration(data.narration || '');
        setIsLabApproved(data.isLabApproved || false);
        
        // Populate fabrics data
        if (data.fabrics && data.fabrics.length > 0) {
          setFabrics(data.fabrics.map((fabric: any, index: number) => ({
            ...fabric,
            id: fabric.id || index + 1
          })));
        }
        
        // Populate processes data
        if (data.processes && data.processes.length > 0) {
          setProcesses(data.processes.map((process: any, index: number) => ({
            ...process,
  ...
  // Submit form handler for updating the record
  const handleSubmit = async () => {
    try {
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: 'Goods inward record updated successfully!',
          status: 'success',
          duration: 5000,
          isClosable: true,
        });
        ...
        toast({
          title: 'Error',
          description: 'Failed to update goods inward record',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
      ...
      console.error('Error updating goods inward record:', error);
      toast({
        title: 'Error',
  ...
  if (loading) {
    return (
      <Container maxW="container.xl" py={5} centerContent>
        <VStack spacing={4}>
          <Spinner size="xl" />
          <Text>Loading goods inward data...</Text>
        </VStack>
      </Container>
    );
  }
  ...
  return (
    <Container maxW="container.xl" py={5}>
      <Box as="form" shadow="md" borderWidth="1px" p={6} borderRadius="md">
        <Heading size="lg" mb={6}>Edit Goods Inward</Heading>
        ...
        <Box mt={6} textAlign="right">
          <Button colorScheme="blue" onClick={handleSubmit} mr={3}>Update Goods Inward</Button>
...
export default EditGoodsInward;