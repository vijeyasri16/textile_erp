...
export default function StockTranferOut() {
  const [no, setNo] = useState('');
  const [date, setDate] = useState('');
  const [reference, setReference] = useState('');
  const [tostockpt, setToStockPt] = useState('');
  ...
  return (
    <Box maxW="1200px" mx="auto" mt={8} p={4} boxShadow="md" borderRadius="md">
      <Heading mb={4}>Stock Transfer-Out</Heading>
      <Grid templateColumns={{ base: '1fr', md: 'repeat(3, 1fr)' }} gap={4}>
        ...
        <FormControl><FormLabel>From Stock Point</FormLabel><Select placeholder="Select" value={tostockpt} onChange={(e) => setToStockPt(e.target.value)} /></FormControl>
