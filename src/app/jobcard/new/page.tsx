...
const JobCardPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.ok) {
        alert('Job Card record added successfully!');
        router.push('/jobcard');
      } else {
        console.error('Failed to add Job Card record', response.status, response.statusText);
      }
    } catch (error) {
      console.error('Error adding Job Card record:', error);
    }
  };

  return (
    <Container maxW="container.xl" py={6}>
      <Heading as="h1" mb={6}>Create Job Card</Heading>
